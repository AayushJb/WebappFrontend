...
import { SubsystemsComponent } from '../settings/subsystems/subsystems.component';
...
import { GetjbComponent } from './getjb/getjb.component';
import { PerformanceComponent } from './performance/performance.component';


import { ReportsService } from 'src/app/services/reports.service';
import { GetjbproComponent } from './getjbpro/getjbpro.component';
...
/*
   ...
   this.http.get<any>(this.hostUrl+"/api/connectprocore").subscribe(response =>{
...
*/
  ...
  OpenLedgerForm(row:Order)
  {
     ...
     //===============Ledger Dialog===============
     this.ledgerdetailsService.getOrderDetails(row.OrderNo,row._id,row.ProjectName,row.Associate);
     ...
     this.dialog.open(LedgerdetailsComponent,dialogConfig)
   ...
   this.wqgformService.getOrder(row._id).subscribe((response)=>{
    let flagT : boolean = false; 
    this.procoreService.createProjectLocation(response).subscribe((newOrderData)=>{
     
      console.log(newOrderData)

      /*
      if(response.Solutions.length===newOrderData.length)
      {
        newOrderData.map(item=>{
          let Floor = item.Floor
          let Space = item.Space
  
          response.Solutions.map(itemOr =>{
           if(Floor===itemOr.Floor&&Space===itemOr.Space)
           {
            itemOr.ProcoreLocationID.push(item);
      ...
      this.http.post<{message : string,orders : Order}>(this.hostUrl+"/api/orders/pushsolutions/"+response._id,response.Solutions).subscribe(responseData => {
          console.log(responseData)
            ...
            //===============Ledger Dialog===============
             this.ledgerdetailsService.getOrderDetails(row.OrderNo,row._id,row.ProjectName,this.UserFullName);
             const scrollStrategy = this.overlay.scrollStrategies.reposition();
             const dialogConfig = new MatDialogConfig();
             dialogConfig.disableClose =true;
             dialogConfig.autoFocus =true;
  ...
  userValuechanges(){
    ...
    this.notloaded = true;
     ...
     this.subscription.push(this.form.get('User').valueChanges.subscribe((response)=>{
      ...
      this.parentSubject.next(response);
      ...
      this.subscription.push(this.form.get('User').valueChanges.subscribe((response)=>{
       this.projectsService.getProjects(response)
        this.projectsService.getOrderUpdateListener().subscribe(projectData =>{
          this.orders = [];
   ...
   statusValuechanges(){
    ...
    this.subscription.push(this.form.get('Status').valueChanges.subscribe((response)=>{
      this.projectsService.getProjects(this.form.value.User)
 ...
 RefreshEdits(row:Order)
 {
  ...
  this.procoreService.UpdateWinProcoreProject(row).subscribe((response)=>{
    ...
    this.http.post<{message : string, orderId : string}>(this.hostUrl+"/api/orders/createnew",response).subscribe(responseData => {
 ...
 OnAdvanceReceived(row :Order)
 {
  ...
  this.commericialwinservice.getOrderDetails(row.OrderNo,row._id,row.ProjectName,this.UserFullName,row.FinalAmount);
  const scrollStrategy = this.overlay.scrollStrategies.reposition();
  const dialogConfig = new MatDialogConfig();
  dialogConfig.disableClose =true;
  dialogConfig.autoFocus =true;
  ...
  dialogConfig.scrollStrategy =  this.overlay.scrollStrategies.reposition();
  this.CWdialog.open(CommercialwinComponent,dialogConfig)
 ...
 OnHandOverOrder(row :Order)
 {
  ...
  this.getjbservice.getOrderDetails(row);
  this.handoverservice.getOrderDetails(row.OrderNo,row._id,row.ProjectName,this.UserFullName,row.FinalAmount)
  const scrollStrategy = this.overlay.scrollStrategies.reposition();
  const dialogConfig = new MatDialogConfig();
  dialogConfig.disableClose =true;
  dialogConfig.autoFocus =true;
  ...
  dialogConfig.scrollStrategy =  this.overlay.scrollStrategies.reposition();
  this.HOdialog.open(HandoverComponent,dialogConfig)

 }


 OnProjectDetails(row : Order)
 {

  this.punchService.getOrderDetails(row);
  const scrollStrategy = this.overlay.scrollStrategies.reposition();
  const dialogConfig = new MatDialogConfig();
  dialogConfig.disableClose =true;
  dialogConfig.autoFocus =true;
  ...
  dialogConfig.scrollStrategy =  this.overlay.scrollStrategies.reposition();
  this.PDdialog.open(ProjectdetailsComponent,dialogConfig)
